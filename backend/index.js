...
app.get("/getTestData", async (req, res) => {
  ...
  for (let i = 0; i < creatorSize; i++) {
    for (let j = 0; j < audienceSize; j++) {
      if (Math.random() >= 0.7) {
        await Subscription.subscribeUser(
          creatorList[i]._id,
          audienceList[j]._id
        );
        subscriptionList.push({
          creator_id: creatorList[i]._id,
          audience_id: audienceList[j]._id,
        });
      }
    }
  }
  ...
  const allText = fs.readFileSync("./fakePosts.txt", "utf8");
  ...
  while (fakePostsLib.length !== 0) {
    const content = fakePostsLib.pop();
    const creatorIndex = Math.floor(Math.random() * creatorSize);
    const post = await Post.createPost(
      creatorList[creatorIndex]._id,
      content,
      null
    );
    postList.push(post);
  ...
  for (let i = 0; i < audienceList.length; i++) {
    for (let j = 0; j < postList.length; j++) {
      if (Math.random() >= 0.95) {
        const like = await Like.likePost(postList[j]._id, audienceList[i]._id);
        likeList.push(like);
      }
    }
  }
  ...
  writeStream.write(
    "_id\t\t\t\t\t\t\tusername\temail\t\t\t\tpassword\t\tisWriter\t\n"
  );
  audienceList.forEach((audience) => {
    writeStream.write(
      `${audience._id}\t${audience.username}\t\t${audience.email}\t\t${audience.password}\t${audience.is_writer}\t\n`
    );
  });
  ...
  creatorList.forEach((creator) => {
    writeStream.write(
      `${creator._id}\t${creator.username}\t\t${creator.email}\t\t${creator.password}\t${creator.is_writer}\t\n`
    );
  });
  ...
  subscriptionList.forEach((subscription) => {
    writeStream.write(
      `${subscription.creator_id}\t\t\t${subscription.audience_id}\t\n`
    );
  });
