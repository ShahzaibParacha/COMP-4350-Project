...
describe('Like schema', function () {
	it('should not accept if user_id is not given', function () {
		const like = new Like({ post_id: new mongoose.mongo.ObjectID() });
	...
	it('should not accept if post_id is not given', function () {
		const like = new Like({ user_id: new mongoose.mongo.ObjectID() });
	...
	it('should accept if both user_id and post-id are given', function () {
		const like = new Like({ user_id: new mongoose.mongo.ObjectID(), post_id: new mongoose.mongo.ObjectID() });
