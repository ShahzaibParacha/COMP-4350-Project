...
async function getCreatorPageByUserId (userId, pageNum, pageSize) {
	if (ParamValidation.isValidObjectId(userId) === false) { return null; }
...
async function updateIsNotification (creatorId, audienceId, notificationSetting) {
	if (ParamValidation.isValidObjectId(audienceId) === false || ParamValidation.isValidObjectId(creatorId) === false) { return false; }

	const result = await Subscriber.updateOne({ audience_id: audienceId, creator_id: creatorId }, { receive_notification: notificationSetting });
	return result.ok === 1;
}
...
async function subscribeUser (creatorId, audienceId) {
	...
	const result = await Subscriber.create(subscribeRelation);
	return result !== null;
}
...
async function cancelSubscribeRelation (creatorId, audienceId) {
	...
	const result = await Subscriber.deleteOne({ creator_id: creatorId, audience_id: audienceId });
	return result.ok === 1;
}

async function getSubscription (creatorId, audienceId) {
	if (ParamValidation.isValidObjectId(audienceId) === false || ParamValidation.isValidObjectId(creatorId) === false) { return null; }
